...
export async function buildVegLibrary(
  ...
  for (let ci = 0; ci < TREE_SPECIES.length; ci++) {
    ...
    progress(0.3 + 0.25 * ((ci + 1) / TREE_SPECIES.length), `식생: ${sp.id} 풀`);
  }

  // ---- tree impostors (variant 0 R1 geometry, relightable octahedral) -------
  progress(0.56, '식생: 옥타헤드럴 임포스터 캡처 중');
  ...
  for (let ci = 0; ci < TREE_SPECIES.length; ci++) {
    ...
    progress(0.56 + 0.18 * ((ci + 1) / TREE_SPECIES.length), `식생: 임포스터 ${sp.id}`);
  }

  // ---- understory: shrubs / fern / flowers (R1 only) -------------------------
  progress(0.76, '식생: 하층 풀 (관목/양치/꽃)');
  const underSpecies = [
