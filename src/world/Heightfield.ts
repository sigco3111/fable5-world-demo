...
export class Heightfield {
  ...
  static async generate(
    ...
    progress(0.04, `지형: ${cfg.heightRes}² 높이맵 합성 중`);
    ...
    progress(0.08, `지형: ${cfg.simRes}² 침식 그리드 합성 중`);
    const synthSim = await runHeightSynthesis(renderer, cfg.simRes, mp);

    progress(0.1, `지형: 침식 적용 중 (${cfg.erosionIters}회 반복)`);
    const erosion = await runErosion(renderer, synthSim.height, synthSim.hardness, {
      ...
      onProgress: (d, t) => progress(0.1 + 0.45 * (d / t), `지형: 침식 ${d}/${t}`),
    });
    ...
    progress(0.7, '지형: 침식된 필드 합성 중');
    await hf.composeEroded(renderer, synthSim.height, erosion.eroded);

    progress(0.82, '지형: 파생 맵 계산 중');
    await hf.rebuildDerivedMaps(renderer);
    await hf.buildFieldsTex(renderer);

    progress(0.88, '지형: 바이옴 + 설분류');
    if (!hf.fieldsTex) throw new Error('필드 텍스처 누락 — 바이옴 패스 진입 전');
