...
export async function buildTerrainScene(ctx: WorldContext): Promise<void> {
  ...
  ctx.progress(0.93, '하늘: 대기 LUT 굽기');
  ...
  ctx.progress(0.95, 'GI: 조도 프로브 수집');
  ...
  ctx.progress(0.97, '하늘: 구름 노이즈 굽기');
  ...
  ctx.progress(0.98, '포스트: 파이프라인 구성');
