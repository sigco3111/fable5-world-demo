...
export async function buildGalleryScene(ctx: WorldContext): Promise<void> {
  ...
  ctx.progress(0.08, '갤러리: 잎 아틀라스 캡처 중');
  ...
  ctx.progress(0.09, '갤러리: 나무껍질 합성 중');
  ...
  const GZ = ROW_Z.ground;
  {
    ...
    exhibit(-26, GZ + 2, '지면 타일 2.4 m', '자갈+가지+조각+낙엽', { pedestal: false });
    ...
    if (fernAtlas) {
      ...
      exhibit(-9, GZ + 2.5, '양치 ×3', '잎 로제트', { pedestal: false });
    }

    // flower patches
    const flowerKinds: { kind: FlowerKind; color: { r: number; g: number; b: number }; n: number; label: string }[] = [
      { kind: 'umbel', color: { r: 0.75, g: 0.75, b: 0.7 }, n: 14, label: '흰 산형화' },
      { kind: 'bell', color: { r: 0.28, g: 0.14, b: 0.5 }, n: 14, label: '종꽃' },
      { kind: 'daisy', color: { r: 0.85, g: 0.72, b: 0.12 }, n: 18, label: '노란 데이지' },
    ];
    let fx = 0;
  ...
  for (let i = 0; i < decays.length; i++) {
    ...
    m.receiveShadow = true;
    engine.scene.add(m);
    exhibit(-22 + i * 9, DZ + 2.5, `썩은 통나무 (${decays[i]})`, `${log.length.toFixed(1)} m`, { pedestal: false });
  }
  {
  ...
  exhibit(11, DZ + 2.5, '그루터기 ×2', '뿌리 플레어, 들쭉날쭉한 윗면', { pedestal: false });

  // ---- hero row: mesh-foliage hero trees (>=100k tris floor) ------------------
  ctx.progress(0.96, '갤러리: 주연 나무');
  await new Promise((r) => setTimeout(r, 0));
  {
    const HZ = ROW_Z.hero;
    ...
    for (const sp of heroSpecs) {
      ...
      const at = exhibit(hx, HZ, `주연 ${sp.label}`, `${(built.stats.tris / 1000).toFixed(0)}k tris (메시 잎)`);
  ...
  ctx.progress(0.97, '갤러리: 임포스터 캡처 중');
  await new Promise((r) => setTimeout(r, 0));
  {
    ...
    if (sp && atlas0) {
      ...
      exhibit(-150, ROW_Z.trees, '임포스터 미리보기', '8×8 옥타 캡처', { pedestal: false });
    }
  }

  // ---- post stack (no clouds in the gallery) ----------------------------------
  ctx.progress(0.95, '갤러리: 포스트 파이프라인');
