...
export class Hud {
  ...
  private render(): void {
    ...
    const lines: string[] = [
      `LAAS  시드=${this.params.seed} 씬=${this.params.scene} 시각=${this.params.timeOfDay}`,
      `${s.fps.toFixed(0)} fps  ${s.frameMs.toFixed(2)} ms (p95 ${s.frameMsP95.toFixed(2)})`,
      `드로우콜 ${fmt(s.drawCalls)}  삼각형 ${fmt(s.triangles)}`,
      `GPU 렌더 ${s.gpuPasses['render']?.toFixed(2) ?? '–'} ms  컴퓨트 ${s.gpuPasses['compute']?.toFixed(2) ?? '–'} ms`,
      `카메라 ${c.x.toFixed(1)}, ${c.y.toFixed(1)}, ${c.z.toFixed(1)}`,
    ];
    ...
    lines.push('—', 'F3 HUD · V 걸기/비행 · 1-9 북마크 · F 플라잇루 · P 포즈');
